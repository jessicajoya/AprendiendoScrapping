...
const fetchValuesCronista = async () => {
  try {
    ...
    let buy_priceData2 = await page.evaluate(() => {
      return document.querySelector(".buy-value")!.textContent;
    });

    let sell_priceData2 = await page.evaluate(() => {
      return document.querySelector(".sell-value")!.textContent;
    });

    let objectValuesCronista = {
